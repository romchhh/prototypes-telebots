...
const phoneDisplay = BRAND.phone.replace(/^\+1\s*/, '')
    ...
    <span className={`${styles.logo} ${className ?? ''}`}>
      <span className={styles.logoMark} aria-hidden="true">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
          <path d="M12 3.2l1.15 4.1 3.05-2.95-1.7 4.4 4.4-1.15-2.95 3.05 4.1 1.15-4.1 1.15 2.95 3.05-4.4-1.15 1.7 4.4-3.05-2.95L12 20.8l-1.15-4.1-3.05 2.95 1.7-4.4-4.4 1.15 2.95-3.05L3.95 12l4.1-1.15-2.95-3.05 4.4 1.15-1.7-4.4 3.05 2.95L12 3.2z" fill="currentColor" />
        </svg>
      </span>
      <span className={styles.logoText}>
        <span className={styles.logoWord}>{BRAND.shortName}</span>
        <span className={styles.logoSub}>Cleaning Co.</span>
      </span>
    </span>
  )
}
  ...
  return (
    <>
      <nav className={`${styles.nav} ${transparent && !scrolled ? styles.transparent : styles.solid}`}>
        <a href="/lumina-clean" className={styles.brand}>
          <Logo />
        </a>

        <div className={styles.center}>
          <a href="#services">{t.nav.services}</a>
          <a href="#book">{t.nav.areas}</a>
          <a href="#book">{t.nav.about}</a>
        </div>
      ...
      <div className={`${styles.drawer} ${menuOpen ? styles.open : ''}`} role="dialog" aria-modal="true">
        <div className={styles.drawerTop}>
          <a href="/lumina-clean" className={styles.drawerBrand} onClick={() => setMenuOpen(false)}>
            <Logo />
          </a>
        ...
        <nav className={styles.drawerNav} aria-label={t.nav.mobileNav}>
          ...
          <a href="#book" onClick={() => setMenuOpen(false)}>{t.nav.areas}</a>
          <a href="#book" onClick={() => setMenuOpen(false)}>{t.nav.about}</a>
        </nav>
        <a href="#book" className={styles.drawerCta} onClick={() => setMenuOpen(false)}>
          {t.nav.cta}
        </a>
