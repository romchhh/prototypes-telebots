...
export default function Navbar({ transparent = false }: { transparent?: boolean }) {
  ...
  const primary = NAV.filter((item) =>
    ['Про нас', 'Ресторани', 'Готелі', 'SPA', 'Вакансії', 'Контакти'].includes(item.label)
  )
  ...
  return (
    <>
      <nav className={`${styles.nav} ${onHero ? styles.transparent : styles.solid}`}>
        <a href="/cartel" className={styles.brand} aria-label="Cartel">
          <Image
            src={onHero ? BRAND.logoWhite : BRAND.logoBlack}
            alt="Cartel"
            width={160}
            height={36}
      ...
      <div className={`${styles.drawer} ${menuOpen ? styles.open : ''}`} role="dialog" aria-modal="true">
        <div className={styles.drawerTop}>
          <a href="/cartel" className={styles.drawerBrand} onClick={() => setMenuOpen(false)} aria-label="Cartel">
            <Image
              src={BRAND.logoBlack}
              alt="Cartel"
              width={160}
        ...
        <a href="#kontakt" className={styles.drawerCta} onClick={() => setMenuOpen(false)}>
          <span>Доставка та бронювання</span>
